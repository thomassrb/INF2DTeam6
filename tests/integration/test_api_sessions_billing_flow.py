...
def test_billing_after_session_shows_item_for_user_and_admin(client: TestClient):
    ...
    r_user_bill = client.get("/api/billing", headers={"Authorization": f"Bearer {user_token}"})
    if r_user_bill.status_code in (404, 405):
        pytest.skip("/api/billing endpoint not implemented")
    assert r_user_bill.status_code == 200, r_user_bill.text
    items = r_user_bill.json()
    assert isinstance(items, list)
    assert any((i.get("session") or {}).get("licenseplate") == plate for i in items)

    r_admin_bill = client.get(
        f"/api/billing/{username}",
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    if r_admin_bill.status_code in (404, 405):
        pytest.skip("/api/billing/{username} endpoint not implemented")
    assert r_admin_bill.status_code == 200, r_admin_bill.text
    admin_items = r_admin_bill.json()
    assert isinstance(admin_items, list)
    assert any((i.get("session") or {}).get("licenseplate") == plate for i in admin_items)
